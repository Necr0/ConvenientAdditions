...
public class ItemSaturationRing extends CAItem implements IBauble {
    ...
    public void onWornTick(ItemStack itemstack, EntityLivingBase player) {
        ...
        if (s.needFood()&&p.ticksExisted%1100==0){
            s.setFoodLevel(Math.min(p.getFoodStats().getFoodLevel()+1+(p.world.rand.nextInt(8)==0?1:0),20));
            s.setFoodSaturationLevel(s.getSaturationLevel()+2f+(p.world.rand.nextFloat()*2));
