...
public class BlockEnderProof extends CABlock implements IEnderProof {
    public BlockEnderProof() {
        super(ModConstants.BlockNames.enderProofBlock, Material.ROCK);
    ...
    public void onEnderTeleport(EnderTeleportEvent e){
        ...
        List<BlockPos> l=rayTraceBlocks(ent.world,new Vec3d(ent.posX,ent.posY,ent.posZ),new Vec3d(e.getTargetX(),e.getTargetY(),e.getTargetZ()),false,new FloodFill.BlockMatcher(3).set(IEnderProof.class));
        if(l.size()>0){
            e.setCanceled(true);
        }
    }
